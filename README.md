...
Реализован сервис, который по кнопке START SERVICE запускает камеру (ее видно на рисунке ниже), которая начинает в фоновом режиме делать снимки каждую секунду. Эти фогографии отдаются модулю с НС, определяющему показываемый жест. В файл JSON сохраняются для каждого "умного" устройства соответствия жестов и команд. Окно свободно можно перемещать по экрану. При сворачивании приложения окно остается видимым для просмотра поподаемых в ракурс объектов.
